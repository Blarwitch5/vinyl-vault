...
export function logout(): void {
  try {
    ...
    document.cookie = `${TOKEN_COOKIE_NAME}=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; secure; httpOnly; sameSite=strict`;
...
function setAuthToken(token: string): void {
  ...
  document.cookie = `${TOKEN_COOKIE_NAME}=${token}; expires=${expires.toUTCString()}; path=/; secure; httpOnly; sameSite=strict`;
}
